    ...
    def predict(self, target_list, tokenizer, model, num_generate=20, batch_size=256, num_optim_tokens=None,
        ...
        for target_index, target in tqdm(list(enumerate(target_list))):
            ...
            while retry:
                ...
                best_new_adv_prefix = get_goodinit(tokenizer, num_optim_tokens, device)
                not_find_times = 0
                for i in range(num_steps):
                    adv_slice = slice(0, len(best_new_adv_prefix))
                    target_slice = slice(len(best_new_adv_prefix), len(best_new_adv_prefix) + len(target_ids))
                    loss_slice = slice(target_slice.start - 1, target_slice.stop - 1)
                    new_adv_ids, is_success, is_find, current_loss, p_loss, scores = \
                        self.compute(best_new_adv_prefix, target_ids, adv_slice, target_slice, loss_slice, tokenizer,
                                     model,

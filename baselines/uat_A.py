    ...
    def predict(self, targets, tokenizer, model,  mode, num_generate=20, num_optim_tokens=8, topk=3,
        ...
        for target_index, target in tqdm(list(enumerate(targets))):
            ...
            for _ in range(num_generate): # total number of subword pieces in the GPT-2 model
                ...
                trigger_tokens = torch.tensor(verbose[num], device=device)
                # loss_list = []
                ...
                for t, gene_len in enumerate(new_generate_lens):
                    ...
                    best_new_generate_tokens = trigger_tokens[start_len:start_len + gene_len]
                    ...
                    model.zero_grad()
                    loss, w_loss, _ = self.get_loss(model, best_prefix, target_tokens, device, t, -1)
                    best_loss = float(loss)
                    while counter < len(best_prefix):
                        for token_to_flip in range(0, len(best_prefix)):  # for each token in the trigger
                            self.extracted_grads = []
                            # w_loss.backward()
                            loss.backward()
                            ...
                            candidates = hotflip_attack(averaged_grad, embedding_weight,
                                                        [best_prefix[token_to_flip]],
                                                        increase_loss=False, num_candidates=topk, device=device)[0]

                            trigger_tokens_batch = torch.tensor(best_prefix, device=device).unsqueeze(0).repeat(
                                len(candidates), 1)
                            ...
                            if curr_best_loss < best_loss:
                                ...
                                best_prefix = deepcopy(trigger_tokens_batch[cur_best_index])
                            elif counter == len(best_prefix):
                                break
                            else:
                                counter = counter + 1

                            loss, w_loss, is_end = self.get_loss(model, trigger_tokens, target_tokens, device, t, small_epoch)
                            # _, w_loss, is_success = self.get_loss(model, best_prefix, target_tokens, device, t, small_epoch)
                            if is_success:
                                break
                    # loss_list.append(best_loss)
                    if is_success:
                        break
                optim_prompts.append(tokenizer.decode(best_prefix))

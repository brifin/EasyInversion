...
class GCG(TrojanDetector):
    ...
    def predict(self, targets, tokenizer, model,  mode, num_generate=20, batch_size=256, num_optim_tokens=None,
        ...
        for target_index, target in tqdm(list(enumerate(targets))):
            ...
            while j < num_generate:
                if should_change:
                    # adv_input_ids = np.random.randint(tokenizer.vocab_size, size=len(verbose[target_index, j, :]))
                    adv_input_ids = np.random.randint(tokenizer.vocab_size, size=len(verbose[num]))
                    adv_input_ids = torch.tensor(adv_input_ids).to(device)
                    should_change = False
                else:
                    # adv_input_ids = verbose[target_index, j, :].clone()
                ...
                not_find_times = 0
                best_new_adv_prefix = adv_input_ids
                is_success = False
                for i in range(num_steps):
                    adv_input = best_new_adv_prefix
                    adv_slice = slice(0, len(best_new_adv_prefix))
                    target_slice = slice(len(adv_input), len(adv_input) + len(target_ids))
                    loss_slice = slice(target_slice.start - 1, target_slice.stop - 1)
                    new_adv_ids, is_success, is_find, current_loss, p_loss, scores = \
                        self.compute(adv_input, target_ids, adv_slice, target_slice, loss_slice, tokenizer, model,
                                         i, device, batch_size, topk)
                    if not is_find:
                        if not_find_times < 5:
                            not_find_times += 1
                            continue
                    ...
                    if is_success:
                        break
                    best_new_adv_prefix = new_adv_ids

                if not_find_times >= 5:
                    should_change = True
                    continue
                best_new_adv_prefix_str = tokenizer.decode(best_new_adv_prefix, skip_special_tokens=True)
